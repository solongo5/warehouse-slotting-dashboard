...
<div style="background:linear-gradient(90deg,#0f1523 0%,#141929 100%);border-bottom:1px solid #1e2d4a;padding:14px 24px;margin:-1rem -2rem 1.5rem -2rem;display:flex;align-items:center;justify-content:space-between;min-width:0;overflow:hidden;">
    <div style="display:flex;align-items:center;flex-wrap:nowrap;white-space:nowrap;overflow:hidden;min-width:0;">
        <span style="font-family:'IBM Plex Mono',monospace;font-size:13px;font-weight:600;color:#00c9a7;letter-spacing:0.08em;text-transform:uppercase;flex-shrink:0;">KEURIG DR PEPPER</span>
        <span style="color:#334155;margin:0 10px;flex-shrink:0;">|</span>
        <span style="font-family:'IBM Plex Mono',monospace;font-size:13px;color:#94a3b8;letter-spacing:0.05em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">SUMNER DC — WAREHOUSE RE-SLOTTING DASHBOARD</span>
        <span style="color:#334155;margin:0 10px;flex-shrink:0;">|</span>
...
<table style="width:100%;font-size:12px;border-collapse:collapse;color:#94a3b8;">
  ...
  <tr>
    <td style="color:#e2e8f0;">Z1 Prime</td>
    <td align="right" style="color:#52c41a;">885</td>
    <td align="right" style="color:#f59e0b;">83</td>
    <td align="right" style="color:#ef4444;">13</td>
    <td align="right">981</td>
  </tr>
  <tr>
    <td style="color:#e2e8f0;">Z2 Secondary</td>
    <td align="right" style="color:#52c41a;">715</td>
    <td align="right" style="color:#f59e0b;">198</td>
    <td align="right" style="color:#ef4444;">70</td>
    <td align="right">983</td>
  </tr>
  <tr>
    <td style="color:#e2e8f0;">Z3 Reserve</td>
    <td align="right" style="color:#52c41a;">61</td>
    <td align="right" style="color:#f59e0b;">0</td>
    <td align="right" style="color:#ef4444;">0</td>
    <td align="right">61</td>
  </tr>
  <tr>
    <td style="color:#e2e8f0;">DR Flex</td>
    <td align="right" style="color:#52c41a;">0</td>
    <td align="right" style="color:#f59e0b;">0</td>
    <td align="right" style="color:#ef4444;">0</td>
    <td align="right">0</td>
  </tr>
...
prime_misplaced = df[
    (df["ABC_Class"] == "A") & (df["Zone"] != "Prime") & (df["Needs_Relocation"])
]
low_in_prime = df[
    (df["ABC_Class"] == "C") & (df["Zone"] == "Prime")
]


# =========================================================
# KPI CARDS  (rendered via components.html to avoid f-string CSS issues)
# =========================================================
cards_data = [
...
with b2:
    st.markdown(
        ...
        "A-class SKUs should be prioritized for Prime zones, while lower-priority inventory can be shifted to secondary storage.</div>",
        unsafe_allow_html=True
    )
...
# =========================================================
st.markdown(
    "<div style='font-family:IBM Plex Mono,monospace;font-size:11px;color:#00c9a7;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:4px;'>&#9638; Recommended Warehouse Bin Map</div>"
    "<div style='font-size:13px;color:#475569;margin-bottom:12px;'>Synthetic lane-level view — bin occupancy and ABC class distribution (simulated from operational logic)</div>",
    unsafe_allow_html=True
)
...
def generate_synthetic_map(seed=42):
    random.seed(seed)
    sku_pool = df_original[["SKU_ID", "ABC_Class", "Zone"]].copy()
    ...
    zone_occ = {"Zone 1 Prime": 0.065, "Zone 2 Secondary": 0.17, "Zone 3 Reserve": 0.02}
    zone_blk = {"Zone 1 Prime": 0.01,  "Zone 2 Secondary": 0.06, "Zone 3 Reserve": 0.0}

    rows = []
    for zone, lanes in zone_lanes.items():
        occ_rate = zone_occ[zone]
        blk_rate = zone_blk[zone]
        zone_short = zone.split()[1]  # Prime / Secondary / Reserve
        ...
        for lane in lanes:
            for i in range(48):
                ...
                if r < blk_rate:
                    ...
                    status = "Occupied"
                    picked = pool.sample(1).iloc[0]
                    abc = picked["ABC_Class"]
                    sku = picked["SKU_ID"]
                else:
...
def get_bin_color(status, abc):
    ...
    return color_map.get(str(abc).strip().upper(), "#1e40af")
...
def render_bin_map(map_df):
    ...
    for zone in zones:
        ...
        if zone_df.empty:
            continue
        inner += f'<div class="zone-header">{zone}</div><div class="lanes-row">'
        for lane in zone_df["Lane"].unique():
...
.map-wrap { padding:0; }
.zone-header { font-size:11px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;color:#00c9a7;margin:20px 0 10px 0;padding-bottom:6px;border-bottom:1px solid #1e2235; }
...
components.html(render_bin_map(map_df), height=680, scrolling=True)
...
i1.markdown(kpi_card("A-Class Outside Prime", str(len(prime_misplaced)), "#ef4444", "SKUs to pull into Zone 1"), unsafe_allow_html=True)
i2.markdown(kpi_card("C-Class in Prime",      str(len(low_in_prime)),    "#f59e0b", "Prime space being used"),   unsafe_allow_html=True)
...
st.markdown(
    ...
    f"Prioritize the {misaligned} relocation candidates first, especially A-class SKUs outside Prime zones, "
    f"instead of re-slotting the entire warehouse.</div>",
    unsafe_allow_html=True
)
...
priority_cols = [c for c in ["SKU_ID","ABC_Class","Speed_Class","Zone","Current_Zone",
                               "Recommended_Zone","Current_Location","Movements",
                               "Stock_Qty","Needs_Relocation","Optimal_Location"]
...
# =========================================================
# BEFORE VS AFTER  (pure HTML table — no st.dataframe)
# =========================================================
st.markdown(
